                            ...
                            <form action="{{ route('posts.update', ['post' => $post->id]) }}" method="POST"
                                ...
                                <div class="form-group">
                                    <div class="form-check mb-3">
                                        ...
                                        <label class="custom-control-label" for="information">{{ __("Información")
                                            }}</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="post_type_id" id="evaluation"
                                            value="2">
                                        <label class="custom-control-label" for="evaluation">{{ __("Evaluación")
                                            }}</label>
                                    </div>
                                    @error('post_type_id') <div class="text-danger">{{ $message }}</div> @enderror
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-check">
                                    ...
                                    <label class="custom-control-label" for="{{ $file->name_file }}">{{
                                        $file->name_file }}</label>
                                </div>
                                @endforeach
                                <div class="form-group">
                                    <label class="border-2 border-gray-200 p-3 w-full block rounded cursor-pointer my-2"
                                        for="customFile1" x-data="{ files: null }">
                                        <input type="file" class="sr-only" name="files[]" id="customFile1" multiple="true"
